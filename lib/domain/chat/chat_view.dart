          ...
          child: FutureBuilder(
            ...
            builder: (BuildContext context, AsyncSnapshot<bool> snapshot) {
              if(snapshot.connectionState == ConnectionState.waiting){
                ...
                debugPrint("[Error]:: ChatView snapshot error : ${snapshot.error}");
...
//FutureBuilder의 fetch 함수에서 채팅창 statusIndex 받아옵니다
  Widget _temp(int index){
    ...
    switch (index) {//초기 index는 2입니다
      ...
      case 4:
        if(statusIndex > 4){ // 공급자가 입고를 완료한 경우
          return _showAlert1();
        }
  ...
  Widget _requestRental1(){
    ...
    return _bubbleContainer(
      widget: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("O 북박스 장소", style: TextStyle(fontWeight: FontWeight.bold),),
          SizedBox(height: 4,),
          Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
          ...
          SizedBox(height: 8,),
          Text(rowBookbox["B_STORE_NAME"], style: TextStyle(fontWeight: FontWeight.bold),),
          Text(widget.row["B_STORE_ADDRESS"]),
          // SizedBox(height: 12,),
          Container(
            width: 270,
          ),
          Text("O 운영시간", style: TextStyle(fontWeight: FontWeight.bold),),
          Text("- 평일 ${rentalInfo["B_START_1"]} ~ ${rentalInfo["B_END_1"]}"),
          Text("- 주말 ${rentalInfo["B_START_6"]} ~ ${rentalInfo["B_END_6"]}"),
          rentalInfo["B_DAY_CLOSE_H"] == "Y" ? Text("- 공휴일 운영") : Text("- 공휴일 미운영"),
        ],
      ), 
      width: 270);
  }
...
////////////////////////////////////////////////////////
  ...
  Future<void> _reset(String currentStatus) async {
    final ChatRepository repository = ref.read(chatRepositoryProvider);
    await repository.resetStatus(widget.row["B_RENTAL_SEQ"], myUserId, currentStatus);
    // try{
    //   debugPrint(myUserId);
